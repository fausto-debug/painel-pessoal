...
function updateClock() {
  ...
  document.getElementById("greetingText").innerHTML = `<span class="wave">👋</span> ${greet}, ${(profile.name || "Usuário").split(" ")[0]}`;
...
function renderSettings() {
  ...
  document.getElementById("cfgPhoto").value = profile.photo || "";
  const dotsEl = document.getElementById("colorDots");
  dotsEl.innerHTML = Object.entries(ACCENTS).map(([key, hex]) =>
    `<div class="color-dot ${profile.accent === key ? "active" : ""}" style="background:${hex};" onclick="window.setAccent('${key}')"></div>`
  ).join("");
...
async function saveProfile() {
  ...
  const photo = document.getElementById("cfgPhoto").value.trim();
  await updateUserProfile(currentUid, { name, photo });
...
function renderAll() {
  ...
  document.getElementById("sideAvatar").innerHTML = profile.photo
    ? `<img src="${profile.photo}" style="width:100%;height:100%;border-radius:50%;object-fit:cover;">`
